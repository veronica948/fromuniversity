#include"Tree.h"
void Tree::Push(node* &root1,int** & matrix,int* edge,bool hasEdge,int value){
	if(!root1) {
		root1=new node(matrix,edge,hasEdge); 
		return;
	}
	if(edge == 0){
		root1=new node(matrix,edge,hasEdge); 
	...
	if(root1->edge == 0){
		if(hasEdge){
			Push(root1->left,matrix,edge,hasEdge);
			return;
		}
		else{
			Push(root1->right,matrix,edge,hasEdge);
			return;
		}
	}
