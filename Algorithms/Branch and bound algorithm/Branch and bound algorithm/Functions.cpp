...
int Graph::buildNewMatrix(int** matrix) {
	...
	for( int i = 0; i < numberOfVertices; i++){
		for(int j = 0; j < numberOfVertices; j++){
			if( i != j && matrix[i][j] != INFINITY && matrix[i][j] != INFINITY_2 
				&& minInLine[i] != INFINITY && minInLine[i] != INFINITY_2 ){
	...
	for( int i = 0; i < numberOfVertices; i++){
		for(int j = 0; j < numberOfVertices; j++){
			if( i != j && matrix[j][i] != INFINITY && matrix[j][i] != INFINITY_2 
				&& minInColumn[i] != INFINITY && minInColumn[i] != INFINITY_2){
	...
	for(int i = 0; i <numberOfVertices; i++){
		...
		if(minInLine[i] != INFINITY_2)
			r += minInLine[i];
		if( minInColumn[i] != INFINITY_2)
			r += minInColumn[i];
	}
	return r;
}
int Graph::countFine(int firstIndex, int secondIndex, int** & matrix){
	...
	for(int i = 0; i < numberOfVertices; i++) {
		if((minInColumn == INFINITY || matrix[i][secondIndex] < minInColumn) && i != firstIndex && i != secondIndex && matrix[i][secondIndex] != INFINITY
			&& matrix[i][secondIndex] != INFINITY_2)
	...
	for(int i = 0; i < numberOfVertices; i++) {
		if((minInLine == INFINITY || matrix[firstIndex][i] < minInLine) && i != secondIndex && i != firstIndex && matrix[firstIndex][i] != INFINITY
			&& matrix[firstIndex][i] != INFINITY_2)
...
int Graph::findMaxFine(int& firstIndex, int & secondIndex, int** & matrix,int & flag){
	int maxFine = -2;
	int fine;
	flag = 0;
	for( int i = 0; i < numberOfVertices; i++){
		for(int j = 0; j < numberOfVertices; j++){
			if(matrix[j][i] == 0){
				...
				if(fine > maxFine && maxFine != INFINITY) {
					firstIndex = j;
					secondIndex = i;
					maxFine = fine;
				}
...
void Graph::branchAndBoundAlgorithm(){
	...
	while(1) {
		...
		int flag;
		int fine = findMaxFine(firstIndex, secondIndex, leaveWithMinValue->matrix,flag);
		int edge[2] = {firstIndex,secondIndex};
		cout<<edge[0]<<edge[1]<<endl;
		int** matrixWithEdge = buildMatrixWithEdge(leaveWithMinValue->matrix,edge);
		int value = buildNewMatrix(matrixWithEdge);
	    if(value != INFINITY) {
			value += leaveWithMinValue->value;
		}
